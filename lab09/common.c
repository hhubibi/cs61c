...
long long int sum_simd(unsigned int vals[NUM_ELEMS]) {
	...
	for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
		...
		unsigned int sum_array[4];
		for (unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
			__m128i values = _mm_loadu_si128((__m128i *) &vals[i]);
			__m128i cmp_result = _mm_cmpgt_epi32(values, _127);
			values = _mm_and_si128(values, cmp_result);
			sum = _mm_add_epi32(values, sum);
		}
...
long long int sum_simd_unrolled(unsigned int vals[NUM_ELEMS]) {
	...
	for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
		...
		__m128i sum = _mm_setzero_si128();
		unsigned int sum_array[4];
		__m128i values, cmp_result;
		for (unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
			values = _mm_loadu_si128((__m128i *) &vals[i]);
			cmp_result = _mm_cmpgt_epi32(values, _127);
			values = _mm_and_si128(values, cmp_result);
			sum = _mm_add_epi32(values, sum);
